    ...
    output$anovatable <- renderPrint(
      {
       ...
       if(input$t1 == "scheffe"){
          a = aov(respos~tratas)  
          ScheffeTest(a, conf.level = input$nc)
       }
      else
          dic(tratas, respos, quali = input$q1, mcomp = input$t1, sigT = input$nc, sigF = input$nc) 
      })
    ...
    output$anovatable2 <- renderPrint(
      {
       ...
       if(input$t2 == "scheffe"){
         a = aov(respos~tratas)  
         ScheffeTest(a, conf.level = input$nc2)
       }
        else
        dbc(tratas, blocos, respos, quali=input$q2, mcomp=input$t2, sigT=input$nc2, sigF=input$nc2)
